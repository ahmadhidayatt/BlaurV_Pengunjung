...
$res  = mysqli_query($connect,"SELECT * FROM tb_membayar_pendakian WHERE id_boking='$id_boking'") or die(mysqli_error($connect));
...
if($res && mysqli_num_rows($res)>0)
{
    ...
    $res2  = mysqli_query($connect,"SELECT * FROM tb_pengunjung WHERE id_pengunjung ='$id'") or die(mysqli_error($connect));
    $data2 = mysqli_fetch_assoc($res2);

    $userEmail = $data2['email']; // now this is your email id variable for user's email address.
  	$nama = $data2['nama'];	


    $mail = new PHPMailer(true);
try {
    $subject = "Terima Kasih telah Melakukan Pembayaran";
    ...
    <tr>
        <td align="center" valign="top" style="border-collapse: collapse; border-spacing: 0; margin: 0; padding: 0; padding-left: 6.25%; padding-right: 6.25%; width: 87.5%; font-size: 17px; font-weight: 400; line-height: 160%;
            ...
            font-family: sans-serif;" class="paragraph">
                Silahkan Cetak E-Tiketing Perjalanan Anda melalui Website
        </td>
    </tr>
...
<!-- End of SECTION / BACKGROUND -->
 ...
 if(!$mail->Send()) {
	...
	$query = ("UPDATE `tb_membayar_pendakian` SET `status_pembayaran` = 'Menunggu Perjalanan' WHERE `id_boking` = '$id_boking'");
